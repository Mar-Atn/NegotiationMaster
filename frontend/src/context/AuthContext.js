  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      console.log('🔍 AuthContext Initialization:', {
        ...
        userData: user ? JSON.parse(user) : 'No user data'
      })
      ...
      if (token && refreshToken && user) {
        ...
        console.log('⚠️ No valid auth tokens found, logging out')
        dispatch({ type: 'LOGOUT' })
      }
    }
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      console.log('✅ Login successful, storing tokens:', {
        ...
        accessToken: response.data.accessToken ? `${response.data.accessToken.substring(0, 20)}...` : 'No token'
      })
